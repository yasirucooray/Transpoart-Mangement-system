...
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="z-index:3;width:250px" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey" style="font-weight:bold">
    ...
    <a href="{{ url('/bus_shedule') }}" class="w3-bar-item w3-button w3-light-grey"><i class="fa fa-caret-right w3-margin-right"></i>Bus Shedule</a>
          ...
          <form action="{{ url('bus_shedule/save') }}" target="_blank" method="POST">
            ...
            <p><select class="w3-input w3-border" aria-placeholder="select Route"  name="shedule">
                @foreach ($route as $rout )
                    <option class="w3-input w3-border" value={{ $rout->id }} >{{ $rout->id }} -  {{ date('H:i A',strtotime($rout->departure_time)) }}</option>
                @endforeach
            ...
            <p><select class="w3-input w3-border" aria-placeholder="select Route"  name="bus">
                @foreach ($buses as $bus )
                    <option class="w3-input w3-border" value={{ $bus->id }} >{{ $bus->bus_no}}</option>
                @endforeach
