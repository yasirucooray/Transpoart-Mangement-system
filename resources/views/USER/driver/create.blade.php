...
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="z-index:3;width:250px" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey" style="font-weight:bold">
    <a onclick="myAccFunc()" href="javascript:void(0)" class="w3-button w3-block w3-white w3-left-align" id="myBtn">
      ...
      <a href="{{ url('/driver') }}" class="w3-bar-item w3-button w3-light-grey"><i class="fa fa-caret-right w3-margin-right"></i>Drivers List</a>
    ...
    <div class="w3-row-padding">
        <div class="w3-col s12">
          ...
          <form action="{{ url('driver/save') }}" method="POST" target="_blank">
            @csrf
            <p><input class="w3-input w3-border" type="text" placeholder="Driver Name" name="Name" required></p>
            <p><select class="w3-input w3-border" aria-placeholder="select Route"  name="bus">
                @foreach ($buses as $buse )
                    <option class="w3-input w3-border" value={{ $buse->id }} >{{ $buse->bus_no }}</option>
                @endforeach

                </select>

            </p>
            <p><input class="w3-input w3-border" type="text" placeholder="License Number" name="license_no" required></p>
